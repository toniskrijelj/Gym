...
public class PrikaziKlijente extends VBox{

	private static final PrikaziKlijente prikaziKlijente = new PrikaziKlijente();
	...
	private PrikaziKlijente() {
        ...
        Label naslov = new Label("KLIJENTI");
        ...
        tabela.setOnMousePressed(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
        		if(clickDelay.clickable() && tabela.getSelectionModel().getSelectedItem() != null) {
        			KlijentEkran.prikazi(tabela.getSelectionModel().getSelectedItem(), 0);
        		}
	...
	private void popuniListu() {
		...
		for(Korisnik k : Korisnici.lista){
			if(k.idTrener == broj && k.ime.startsWith(search.getText())){
	...
	public static void prikazi(int broj) {
		PrikaziKlijente.broj = broj;
		prikaziKlijente.init();
		HelloApplication.scena.setRoot(prikaziKlijente);
	}
}